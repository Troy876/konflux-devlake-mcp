...
class PRRetestTools(BaseTool):
    ...
    def get_tools(self) -> List[Tool]:
        ...
        return [
            Tool(
                ...
                description="**Comprehensive PR Retest Analysis Tool** - Analyzes all pull requests in a repository within a DevLake project that required manual retest commands (comments containing '/retest'). Provides detailed statistics including: total count of manual retest comments (excluding bot comments), number of PRs affected, average retests per PR, top PRs with most retests (including PR title, URL, number of retests, PR duration, changes, and status), analysis of root causes and failure patterns, breakdown by PR category, timeline visualization data, and actionable recommendations. Focuses on technical patterns, systemic issues, and actionable insights without financial impacts or personal attribution.",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "repo_name": {"type": "string", "description": "Repository name to analyze (e.g., 'integration-service', 'build-service', 'release-service', 'infra-deployments'). Can be partial match from PR URL."},
                        "project_name": {"type": "string", "description": "DevLake project name (e.g., 'Secureflow - Konflux - Global', 'Konflux_Pilot_Team'). Leave empty to search all projects."},
                        "days_back": {"type": "integer", "description": "Number of days back to analyze (default: 90 for 3 months). Leave empty to analyze all available data."},
                        "start_date": {"type": "string", "description": "Start date for analysis (format: YYYY-MM-DD or YYYY-MM-DD HH:MM:SS). Leave empty for no start date limit."},
                        "end_date": {"type": "string", "description": "End date for analysis (format: YYYY-MM-DD or YYYY-MM-DD HH:MM:SS). Leave empty for no end date limit."},
                        "top_n": {"type": "integer", "description": "Number of top PRs to return in detailed analysis (default: 15, max: 50)"},
                        "exclude_bots": {"type": "boolean", "description": "Exclude bot comments from analysis (default: true). Bot detection is based on account_id patterns and comment characteristics."}
                    },
                    "required": []
                }
            )
    ...
    async def call_tool(self, name: str, arguments: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if name == "analyze_pr_retests":
                ...
                result = {
                    "success": False,
                    "error": f"Unknown PR retest tool: {name}"
                }
            ...
            return toon_encode(error_result, {
                "delimiter": ",",
                "indent": 2,
                "lengthMarker": ""
            })
    ...
    async def _analyze_pr_retests_tool(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            repo_filter = ""
            if repo_name:
                repo_filter = f" AND (r.name LIKE '%{repo_name}%' OR r.url LIKE '%{repo_name}%' OR pr.url LIKE '%{repo_name}%')"
            ...
            total_retests = int(float(total_result["data"][0]["total_retests"])) if total_result["success"] and total_result["data"] else 0
            ...
            # Convert MySQL numeric types (Decimal/string) to int
            affected_prs = int(float(affected_result["data"][0]["affected_prs"])) if affected_result["success"] and affected_result["data"] else 0
            ...
            category_query = f"""
                SELECT 
                    CASE 
                        WHEN LOWER(pr.title) LIKE '%bug%' OR LOWER(pr.title) LIKE '%fix%' THEN 'Bug Fixes'
                        WHEN LOWER(pr.title) LIKE '%feat%' OR LOWER(pr.title) LIKE '%feature%' THEN 'Features'
                        WHEN LOWER(pr.title) LIKE '%dep%' OR LOWER(pr.title) LIKE '%dependenc%' THEN 'Dependencies'
                ...
                FROM (
                    ...
                    LEFT JOIN lake.project_mapping pm2 ON r2.id = pm2.row_id AND pm2.`table` = 'repos'
                    WHERE LOWER(REPLACE(REPLACE(TRIM(prc.body), '"', ''), '''', '')) = '/retest'
                        AND prc.body IS NOT NULL
                        AND prc.body != ''
                        {project_filter_subquery}
            ...
            recommendations = self._generate_recommendations(
                total_retests, affected_prs, avg_retests, 
                category_breakdown, pattern_analysis, top_prs
            )
            ...
            return {
                ...
                "executive_summary": {
                    ...
                    "time_period_analyzed": f"{days_back} days" if days_back > 0 else "all available data"
                },
    ...
    def _generate_recommendations(
        ...
        if avg_retests > 3:
            recommendations.append({
                ...
                "recommendation": "Investigate CI/CD pipeline stability. Consider: 1) Reviewing flaky test patterns, 2) Improving test environment reliability, 3) Implementing automatic retry mechanisms for transient failures",
                "impact": "Reduces developer friction and speeds up PR review cycles"
            })

        # Category-specific recommendations
        if category_breakdown:
            max_category = max(category_breakdown, key=lambda x: float(x.get("total_retests", 0) or 0))
            ...
            if max_retests > total_retests * 0.3:  # More than 30% of retests
                recommendations.append({
                    ...
                    "issue": f"'{max_category.get('category')}' category accounts for {int(max_retests)} retests",
                    "recommendation": f"Focus improvement efforts on {max_category.get('category')} PRs. Review common failure patterns in this category and optimize test suites accordingly.",
            ...
            if merged_prs:
                ...
                if avg_retests_val > 2:
                    recommendations.append({
                        ...
                        "recommendation": "Strengthen pre-merge test coverage and ensure all critical paths are tested before merge approval. Consider implementing mandatory test passes before allowing merge.",
                        "impact": "Reduces post-merge issues and improves code quality"
                    })

        # Large change recommendations
        if top_prs:
            large_change_prs = [pr for pr in top_prs if (pr.get("additions", 0) or 0) + (pr.get("deletions", 0) or 0) > 1000]
            if large_change_prs:
                recommendations.append({
                    "priority": "low",
                    "category": "PR Size Management",
                    "issue": f"{len(large_change_prs)} PRs with >1000 lines changed have high retest counts",
                    "recommendation": "Consider breaking large PRs into smaller, more manageable chunks. Smaller PRs are easier to test and review, reducing retest frequency.",
        ...
        if not recommendations:
            recommendations.append({
                ...
                "recommendation": "Continue monitoring retest patterns. Consider implementing automated test retry mechanisms for known flaky tests.",
    ...
    def _analyze_patterns(
        ...
        if pattern_analysis:
            ...
            insights.append(
                f"PRs with status '{max_status.get('status')}' have the highest average retest count "
                f"({avg_retests_val:.2f} retests per PR)"
            )
