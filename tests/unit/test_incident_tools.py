...
class TestIncidentTools:
    ...
    @pytest.mark.asyncio
    async def test_get_incidents_no_filters(self, incident_tools, mock_db_connection, sample_incident_data):
    ...
    @pytest.mark.asyncio
    async def test_get_incidents_with_status_filter(self, incident_tools, mock_db_connection, sample_incident_data):
        """Test getting incidents with status filter."""
        filtered_data = [incident for incident in sample_incident_data if incident["status"] == "DONE"]
        mock_db_connection.execute_query.return_value = {
            "success": True,
    ...
    @pytest.mark.asyncio
    async def test_get_incidents_with_component_filter(self, incident_tools, mock_db_connection, sample_incident_data):
        """Test getting incidents with component filter."""
        filtered_data = [incident for incident in sample_incident_data if incident["component"] == "api-service"]
        mock_db_connection.execute_query.return_value = {
            "success": True,
    ...
    @pytest.mark.asyncio
    async def test_get_incidents_with_days_back_filter(self, incident_tools, mock_db_connection, sample_incident_data):
        """Test getting incidents with days_back filter."""
        mock_db_connection.execute_query.return_value = {
            "success": True,
            "query": "SELECT t1.* FROM lake.incidents t1 ... WHERE t1.created_date >= '2024-01-01 00:00:00'",
            "row_count": 2,
            "data": sample_incident_data
        }
    ...
    @pytest.mark.asyncio
    async def test_get_incidents_with_date_range(self, incident_tools, mock_db_connection, sample_incident_data):
        """Test getting incidents with explicit date range."""
        mock_db_connection.execute_query.return_value = {
            "success": True,
            "query": "SELECT t1.* FROM lake.incidents t1 ... WHERE t1.created_date >= '2024-01-15 00:00:00' AND t1.created_date <= '2024-01-16 23:59:59'",
            "row_count": 2,
            "data": sample_incident_data
        }
    ...
    @pytest.mark.asyncio
    async def test_get_incidents_with_datetime_range(self, incident_tools, mock_db_connection, sample_incident_data):
        """Test getting incidents with full datetime range."""
        mock_db_connection.execute_query.return_value = {
            "success": True,
            "query": "SELECT t1.* FROM lake.incidents t1 ... WHERE t1.created_date >= '2024-01-15 10:00:00' AND t1.created_date <= '2024-01-16 12:00:00'",
            "row_count": 2,
            "data": sample_incident_data
        }
    ...
    @pytest.mark.asyncio
    async def test_get_incidents_with_different_date_fields(self, incident_tools, mock_db_connection, sample_incident_data):
        """Test getting incidents with different date field options."""
        for date_field in ["created_date", "resolution_date", "updated_date"]:
            mock_db_connection.execute_query.return_value = {
                "success": True,
                "query": f"SELECT t1.* FROM lake.incidents t1 ... ORDER BY t1.{date_field} DESC",
                "row_count": 2,
                "data": sample_incident_data
            }
            
            result_json = await incident_tools.call_tool("get_incidents", {"date_field": date_field})
            result = json.loads(result_json)
    ...
    async def test_get_incidents_invalid_date_field(self, incident_tools):
        ...
        result_json = await incident_tools.call_tool("get_incidents", {"date_field": "invalid_field"})
        result = json.loads(result_json)
    ...
    @pytest.mark.asyncio
    async def test_get_incidents_with_limit(self, incident_tools, mock_db_connection, sample_incident_data):
    ...
    @pytest.mark.asyncio
    async def test_get_incidents_combined_filters(self, incident_tools, mock_db_connection, sample_incident_data):
        """Test getting incidents with multiple filters combined."""
        filtered_data = [incident for incident in sample_incident_data 
                        if incident["status"] == "DONE" and incident["component"] == "api-service"]
        
        mock_db_connection.execute_query.return_value = {
            "success": True,
            "query": "SELECT t1.* FROM lake.incidents t1 ... WHERE t1.status = 'DONE' AND t1.component = 'api-service'",
            "row_count": 1,
            "data": filtered_data
        }
    ...
    def test_incident_tool_input_schema(self, incident_tools):
        ...
        expected_properties = [
            "status", "component", "days_back", "start_date", 
    ...
    def test_incident_data_structure_validation(self, sample_incident_data):
        ...
        for incident in sample_incident_data:
            required_fields = [
                "incident_key", "title", "description", "status", 
                "created_date", "component", "url"
            ]
