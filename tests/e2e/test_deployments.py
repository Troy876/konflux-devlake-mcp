...
async def test_llm_list_deployments_with_projects(model: str, mcp_client: ClientSession):
    ...
    messages = [
        ...
        Message(
            ...
            content=(
                ...
                "In your final answer explicitly name, for each deployment, the project and the deployment title."
            ),
        ),
    ]
    expected_values = [
...
@pytest.mark.parametrize("model", models)
@pytest.mark.flaky(max_runs=3)
async def test_llm_failed_deployments_with_failure_time(model: str, mcp_client: ClientSession):
    tools = await get_converted_tools(mcp_client)
    messages = [
        Message(role="system", content=DIRECTIVE_SYSTEM_PROMPT),
        Message(
            role="user",
            content=(
                "List failed deployments. Use only the get_deployments tool (do not run custom SQL). "
                "In the tool call, do NOT set days_back, start_date, end_date, or environment (let the tool defaults apply). "
                "From the returned deployments, select those with result = FAILURE and in your final answer explicitly include, for each: "
                "deployment id, project name, environment, and the finished_date formatted as YYYY-MM-DD HH:MM:SS."
            ),
        ),
    ]
