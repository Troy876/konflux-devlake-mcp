...
def _disable_litellm_background_workers() -> None:
    """Best-effort shutdown/disable of LiteLLM background logging to avoid event loop conflicts in tests."""
    try:
        import litellm
        try:
...
def _summarize_tool_output(tool_name: str, args: Dict[str, Any], raw: str) -> str:
    ...
    def _to_list(value):
        ...
        if isinstance(value, dict):
            for k in ("data", "items", "results", "databases", "tables", "incidents", "deployments", "columns"):
                v = value.get(k)
    ...
    if tool_name and "database" in tool_name.lower() and "table" not in tool_name.lower():
        ...
        if names:
            ...
            return f"There {'is' if len(names)==1 else 'are'} {len(names)} {plural}: {preview}{more}."
    ...
    if tool_name and "schema" in tool_name.lower():
        ...
        if cols and all(isinstance(x, dict) for x in cols):
            ...
            for item in cols[:50]:
                ...
                names.append(f"- {name}{' ('+ctype+')' if ctype else ''}")
    ...
    if seq and all(isinstance(x, dict) for x in seq):
        ...
        return f"Found {len(seq)} items with fields: {keys_text}. Sample:\n\n" + "\n".join(lines) + more
...
async def _finalize_llm_answer(model: str, conversation: List[Message], default_text: str) -> str:
...
def _append_md_log(section: Dict[str, Any]):
...
def _reason_from_expected(assistant_text: str, expected: Optional[List[str]]) -> str:
    if not expected:
...
async def outcome_based_test(
    ...
    asked_texts = [m.content for m in messages if isinstance(m, Message) and getattr(m, "role", "") == "user"]
        ...
        if not tool_calls:
            ...
            if final_answer:
                ...
                _append_md_log({
                    ...
                    "reason": _reason_from_expected(final_answer, expected_keywords) if expected_keywords else "Final answer returned and validated by test assertions",
                })
                return final_answer
            continue
