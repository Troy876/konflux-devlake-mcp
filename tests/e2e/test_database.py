...
async def test_llm_db_connect(model: str, mcp_client: ClientSession):
    ...
    answer = await outcome_based_test(
        model, messages, tools, mcp_client, expected_keywords=["host", "port", "user", "database"]
    )
    text = answer.lower()
...
async def test_llm_list_tables(model: str, mcp_client: ClientSession):
    ...
    messages = [
        ...
        Message(
            role="user",
            content=(
                "List all table names in the 'lake' database and explicitly name each one in your answer."
            ),
        ),
    ]
    required_tables = [
